<?php
      ...
      class Form extends CI_Controller{
            ...
            public function index() {
                  ...
                  if ($verify == $_SESSION['verify']) {
                        if ($this->form_validation->run() == FALSE) {
                              echo "<script>alert('请补全信息!');window.location.href='".site_url('/application/view/applications')."'</script>";
                        } else {
            ...
            public function excel($page=NULL) {
                  $data['student_data'] = $this->form_model->get_data($page);
                  if (empty($data['student_data'])) {
                        show_404();
                  }
                  $this->load->view('application/excel', $data);
            }
            public function excellogo() {
                 $this->excel('logo设计大赛');
            }
            public function excelppt() {
                 $this->excel('ppt演讲大赛');
            }
            public function excelvideo() {
                 $this->excel('视频制作大赛');
            }
